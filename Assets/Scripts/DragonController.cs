...
public class DragonController : FGroundFitter_MovementLook
{
    ...
    protected override void Update()
    {
        ...
        if (flying)
        {
            ...
            tpp.FollowingOffsetDirection = new Vector3(0f, 0f,Mathf.Lerp(tpp.FollowingOffsetDirection.z,-15f,Time.deltaTime * 2f));
        }
        else
        {
            tpp.FollowingOffset = new Vector3(0f, Mathf.Lerp(tpp.FollowingOffset.y,3f,Time.deltaTime * 2f));
            tpp.FollowingOffsetDirection = new Vector3(0f, 0f,Mathf.Lerp(tpp.FollowingOffsetDirection.z,-7f,Time.deltaTime * 2f));
        }
    }
    ...
    protected override void HandleTransforming()
    {
        ...
        else
        {
            ...
            targetFlyRotation = Quaternion.Euler(targetFlyRotation.eulerAngles.x - 15f, targetFlyRotation.eulerAngles.y + RotationOffset, targetFlyRotation.eulerAngles.z);
            ...
            if (Input.GetKey(KeyCode.Z)) verticleFlySpeed = Mathf.Lerp(verticleFlySpeed, -1f, Time.deltaTime);
            else
            {
